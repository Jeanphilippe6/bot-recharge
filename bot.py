...
def init_db():
    ...
    """)
    cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('liquidite', '1000000')")
...
TEXTS = {
    'fr': {
        ...
        'ask_code': "👉 Veuillez envoyer votre **code de recharge {produit}** ci-dessous :",
...
GRILLES_TARIFS = {
    "PCS": {20: 7000, 50: 23000, 100: 53000},
    "Transcash": {20: 8000, 50: 28000, 100: 58000},
}
...
def rating_keyboard(tx_id):
    keyboard = [
        [InlineKeyboardButton(f"⭐ {i}", callback_data=f"rate_{tx_id}_{i}") for i in range(1, 8)]
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def gerer_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        if solde < SEUIL_MIN_RETRAIT:
            ...
            await query.edit_message_text(
                t['retrait_demande'].format(solde=solde),
                parse_mode="Markdown"
            )
        ...
        await query.edit_message_text(
            ...
            f"Gagnez <b>{BONUS_PARRAINAGE} XOF</b> pour chaque ami invité qui effectue sa première transaction validée !\n\n"
            f"🔗 <b>Lien d'invitation :</b>\n<code>{link}</code>\n\n"
        ...
        cursor.execute(
            "SELECT produit, montant_eur, montant_crypto, statut, date_creation, rating FROM transactions WHERE user_id = ? ORDER BY id DESC",
            (user.id,)
        )
        ...
        await query.edit_message_text(f"Service : <b>{html.escape(produit)}</b>", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        ...
        await query.edit_message_text(t['ask_code'].format(produit=produit), parse_mode="Markdown")
...
async def gerer_messages_texte(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if etape == "ATTENTE_CODE":
        ...
        cursor.execute(
            ...
            (user.id, produit, "XOF", montant_eur, montant_crypto, texte, "En attente", now_str)
        )
...
async def gerer_actions_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "valide":
        ...
        if tx:
            ...
            # Message explicatif avec la demande de note sur 7 étoiles
...
async def admin_set_liquidite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Usage: `/liquidite 5000000`", parse_mode="Markdown")
